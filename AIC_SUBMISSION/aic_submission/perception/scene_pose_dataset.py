...
from aic_submission.perception.plug_aware_dataset import _canonical_quat_xyzw
...
def _scene_target(sample_path: Path) -> np.ndarray:
...
def compute_scene_target_stats(samples: list[SampleRef]) -> dict[str, object]:
    """Compute train-set stats for absolute port pose and relative plug pose."""
    ...
    for sample in samples:
        target = _scene_target(sample.path)
        count += 1
        total += target
    ...
    return {
        ...
        "layout": "port_xyz plug_relative_xyz port_quat_xyzw plug_quat_xyzw",
    }


class ScenePoseDataset(Dataset):
    """Image-first dataset for static scene pose plus current plug pose."""
