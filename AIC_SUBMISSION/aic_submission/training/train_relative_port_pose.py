...
def main() -> int:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("AIC_SUBMISSION/runs/relative_port_pose"),
    )
    ...
    target_stats = compute_relative_target_stats(train_samples)

    camera_keys = DEFAULT_CAMERA_KEYS
    task_dim = len(train_samples[0].task_vector)
    train_dataset = RelativePortPoseDataset(
        train_samples,
        target_mean=target_stats["mean"],
        target_std=target_stats["std"],
        camera_keys=camera_keys,
    )
    val_dataset = RelativePortPoseDataset(
        val_samples,
        target_mean=target_stats["mean"],
        target_std=target_stats["std"],
        camera_keys=camera_keys,
    ...
    print(
        f"Training relative multi-camera model on {len(train_samples)} train / "
