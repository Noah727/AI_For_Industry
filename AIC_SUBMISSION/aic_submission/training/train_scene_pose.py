...
def main() -> int:
    ...
    zero_state = not args.use_state
    train_dataset = ScenePoseDataset(
        train_samples,
    ...
    config = {
        ...
        "output_dim": 14,
        "zero_state": zero_state,
        "target_layout": "port_xyz plug_relative_xyz port_quat_xyzw plug_quat_xyzw",
    }
    history = {
        **config,
    ...
    print(
        f"Training scene pose model on {len(train_samples)} train / "
        f"{len(val_samples)} val samples using {device}; zero_state={zero_state}."
    )
