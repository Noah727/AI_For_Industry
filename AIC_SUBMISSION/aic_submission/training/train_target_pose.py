...
from aic_submission.perception.target_pose_dataset import (
    TargetTcpPoseDataset,
    compute_target_pose_stats,
)
...
def main() -> int:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("AIC_SUBMISSION/runs/target_pose_100ep"),
    )
    ...
    parser.add_argument(
        ...
        choices=["basic", "rich"],
        default="rich",
        help="Use rich mode to include target module/rail and port identity.",
    )
    parser.add_argument(
        "--task-filter",
        choices=["all", "sfp", "sc", "sfp_port_0", "sfp_port_1"],
        default="all",
        help="Train a specialist on only one task family.",
    )
    ...
    if not samples:
        raise RuntimeError(
            f"No teacher samples found under {args.root} for filter "
            f"{args.task_filter!r}"
        )
    ...
    target_stats = compute_target_pose_stats(train_samples)

    camera_keys = DEFAULT_CAMERA_KEYS
    task_dim = len(train_samples[0].task_vector)
    train_dataset = TargetTcpPoseDataset(
        train_samples,
        target_mean=target_stats["mean"],
        target_std=target_stats["std"],
        camera_keys=camera_keys,
    )
    val_dataset = TargetTcpPoseDataset(
        val_samples,
        target_mean=target_stats["mean"],
        target_std=target_stats["std"],
        camera_keys=camera_keys,
    ...
    config = {
        ...
        "task_vector_mode": args.task_vector_mode,
        "task_dim": task_dim,
    ...
    print(
        f"Training target-pose model on {len(train_samples)} train / "
