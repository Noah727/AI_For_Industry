...
class PlugAwarePrediction:
    port_relative_xyz: np.ndarray
    plug_relative_xyz: np.ndarray
    port_xyz: np.ndarray
    plug_xyz: np.ndarray
...
class PlugAwarePoseInference:
    """Runtime wrapper for the joint port and plug-position model."""
    ...
    def task_vector(task) -> tuple[float, float, float, float]:
        return (
            1.0 if task.plug_type == "sfp" else 0.0,
            1.0 if task.plug_type == "sc" else 0.0,
            1.0 if task.port_type == "sfp" else 0.0,
            1.0 if task.port_type == "sc" else 0.0,
        )
    ...
    def predict(self, obs, task) -> PlugAwarePrediction:
        ...
        return PlugAwarePrediction(
            port_relative_xyz=port_relative,
            plug_relative_xyz=plug_relative,
            port_xyz=tcp_xyz + port_relative,
